...
func TestFromListSafe(t *testing.T) {
	s := FromListSafe([]int{1, 3, 5, 7, 1, 3, 5})
	if s.Size() != 4 {
...
func TestAddRemoveSafe(t *testing.T) {
	s := NewSafe[int]()
	s.Add(1)
	if s.Size() != 1 {
...
func TestSubtractSafe(t *testing.T) {
	s := FromListSafe([]int{1, 3, 5, 7})
	u := FromListSafe([]int{5, 7, 9, 12})
	r := s.Subtract(u)
...
func TestIntersectionSafe(t *testing.T) {
	s := FromListSafe([]int{1, 3, 5, 7})
	u := FromListSafe([]int{5, 7, 9, 12})
	r := s.Intersection(u)
